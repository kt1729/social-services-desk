              ...
              return (
                ...
                >
                  ...
                  {available && langInfo?.storagePath && (
                    <SignedLink storagePath={langInfo.storagePath} />
                  )}
                </div>
              );
            })}
...
function SignedLink({ storagePath }: { storagePath: string }) {
...
function DocumentPreview({
  ...
  useEffect(() => {
    if (!enStoragePath || doc.type === 'link' || doc.type === 'internal') return;

    let cancelled = false;
    getFileUrl(enStoragePath).then((url) => {
      if (!cancelled) setSignedUrl(url);
    });
    return () => {
      cancelled = true;
    };
  }, [enStoragePath, doc.type]);
