...
export default function DocumentForm({ open, onClose, document: existingDoc }: DocumentFormProps) {
  ...
  const uploadFile = async (file: File, docId: string, lang: LanguageCode): Promise<string> => {
    const storagePath = `documents/${docId}/${lang}/${file.name}`;
    ...
    try {
      ...
      if (isEdit && existingDoc) {
        ...
        for (const lang of SUPPORTED_LANGUAGES) {
          ...
          if (file) {
            const downloadUrl = await uploadFile(file, existingDoc.id, lang.code);
            languages[lang.code] = { available: true, storagePath: downloadUrl };
          }
        }
        ...
        for (const lang of SUPPORTED_LANGUAGES) {
          const file = langFiles[lang.code];
          if (file) {
            const downloadUrl = await uploadFile(file, docRef.id, lang.code);
            languages[lang.code] = { available: true, storagePath: downloadUrl };
            hasUploads = true;
          }
        }
