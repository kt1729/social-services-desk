...
import type { Resource, TranslatedField, CategoryKey, LanguageCode } from '../../shared/types';
...
export default function ResourceForm({ open, onClose, resource }: ResourceFormProps) {
  ...
  const [openDays, setOpenDays] = useState(resource?.openDays ?? '');
  const [openHours, setOpenHours] = useState(resource?.openHours ?? '');
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {activeLang === 'en' && (
          <>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Open Days</label>
                <input
                  value={openDays}
