...
function renderSearchBar(initialRoute = '/search') {
  return render(
...
describe('SearchBar', () => {
  ...
  it('debounces navigation while typing on search page', async () => {
    ...
    // Before debounce fires, no navigation yet (on /search page, debounce applies)
  ...
  it('navigates immediately when typing from non-search page', async () => {
    vi.useRealTimers();
    const user = userEvent.setup();
    renderSearchBar('/');
    ...
    expect(mockNavigate).toHaveBeenCalledWith('/search?q=h');
  });
  ...
  it('shows empty input when not on search page', () => {
    renderSearchBar('/resources/123');
    const input = screen.getByPlaceholderText(
      'Search resources, documents, guests...',
    ) as HTMLInputElement;
