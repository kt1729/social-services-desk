...
| File Storage | Firebase Storage            | Free tier | 5GB storage, 1GB/day download              |
...
| QR Codes     | `qrcode.js` (client-side)   | Free      | No API needed                              |
...
├── openDays: string                  ← NOT translated ("Mon–Fri")
├── openHours: string                 ← NOT translated ("5PM – 8AM")
...
| `openDays / openHours` | ❌ No | Keep simple, use icons on print |
...
│     storagePath: string | null,             ← for uploads (Firebase Storage path)
...
└── documents/
    ...
    └── {documentId}/
        └── en/
            └── winter-shelter-flyer.jpg
```

**Free tier limits:** 5GB storage, 1GB/day download — sufficient for a service desk.
...
service cloud.firestore {
  match /databases/{db}/documents {
    ...
    // Resources: any authenticated volunteer can read/create/update, only admin can delete
...
service firebase.storage {
  match /b/{bucket}/o {
    ...
    // Documents: only authenticated users can read/write
...
Step 5:  Enable Authentication → Email/Password provider
Step 6:  Copy config keys into the app's firebase config
Step 7:  Deploy Firestore security rules (Section 7)
Step 8:  Deploy Storage security rules (Section 8)
...
| Firebase Storage  | 5 GB                 | ~1 GB (PDFs/flyers)       | 5x       |
| Storage downloads | 1 GB/day             | ~100 MB/day               | 10x      |
